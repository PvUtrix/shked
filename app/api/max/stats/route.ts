...
import { getNotificationStats } from '@/lib/max/notifications'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const recentConnections = await prisma.maxUser.findMany({
      where: {
        createdAt: {
    ...
    // Статистика по ролям
    const roleStats = await prisma.maxUser.groupBy({
      by: ['userId'],
      where: {
        isActive: true
      },
    ...
    const [totalUsers, totalGroups, totalSchedules, totalSubjects, activeMaxUsers, notificationsEnabled] = systemStats
    ...
    return NextResponse.json({
      ...
      groupStats: groupStats.map(group => ({
        ...
        maxConnected: group.users.filter(u => u.maxUser?.isActive).length,
        maxPercentage: group._count.users > 0 
          ? Math.round((group.users.filter(u => u.maxUser?.isActive).length / group._count.users) * 100)
          : 0
      })),
      ...
      systemStats: {
        ...
        maxConnectionRate: totalUsers > 0 
          ? Math.round((activeMaxUsers / totalUsers) * 100)
          : 0
      }
    })
  } catch (error) {
