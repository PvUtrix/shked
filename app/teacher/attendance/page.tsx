...
export default async function TeacherAttendancePage({
  ...
  if (!session || !['teacher', 'lector', 'admin'].includes(session.user.role)) {
    redirect('/login')
  }
  ...
  const stats = [
    ...
    },
    {
      ...
      description: 'Средняя по занятиям',
      icon: AlertCircle,
        ...
        <TabsContent value="completed" className="space-y-4">
          {completed.length === 0 ? (
            <Card>
              <CardContent className="pt-6">
                <p className="text-center text-muted-foreground">
                  Нет занятий с отмеченной посещаемостью
                </p>
              </CardContent>
            </Card>
          ) : (
            completed.map((schedule) => {
              const totalStudents = schedule.attendance.length
              const presentStudents = schedule.attendance.filter(a => a.status === 'PRESENT').length
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <p className="text-sm text-muted-foreground">
                        Посещаемость: {presentStudents} из {totalStudents} ({attendancePercent}%)
                      </p>
