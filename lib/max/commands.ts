...
export interface CommandHandler {
  (userId: string, chatId: string, args?: string[]): Promise<string>
}
...
export async function findUserByLinkToken(token: string): Promise<string | null> {
  ...
  const userId = verificationToken.identifier.replace('max_link_', '')
  return userId
}

/**
 * Привязать Max аккаунт к пользователю
 */
export async function linkMaxAccount(
  userId: string,
  maxId: string,
  chatId: string,
  username?: string,
  ...
  try {
    ...
    console.error('Ошибка при привязке Max аккаунта:', error)
    return false
  }
}
...
export async function getUserByMaxId(maxId: string) {
  return await prisma.maxUser.findUnique({
    where: { maxId },
    include: {
      user: {
        include: {
...
export async function handleStart(userId: string, _chatId: string): Promise<string> {
  ...
  if (maxUser) {
    return `Привет, ${maxUser.user.firstName}! 👋
...
Для подключения вашего Max аккаунта к системе:
 ...
 * Команда /link
 */
export async function handleLink(userId: string, chatId: string, args?: string[]): Promise<string> {
...
Пожалуйста, получите новый токен в веб-приложении ШКЕД.`
  ...
  // Привязываем аккаунт
  const success = await linkMaxAccount(
    webUserId,
    userId,
    chatId.toString(),
...
Используйте /link для привязки аккаунта.`
  ...
  const schedules = await getSchedule(maxUser.user.id)
...
Используйте /link для привязки аккаунта.`
  ...
  const schedules = await getSchedule(maxUser.user.id, tomorrow)
...
🔔 Уведомления: ${maxUser.notifications ? 'Включены' : 'Отключены'}
📱 Max ID: ${maxUser.maxId}
👤 Пользователь: ${maxUser.user.firstName} ${maxUser.user.lastName}
🎓 Группа: ${maxUser.user.group?.name || 'не указана'}
...
Используйте /link для привязки аккаунта.`
  ...
  return await processUserQuery(maxUser.user.id, text)
}
