...
/**
 * Получить контекст пользователя для LLM
 */
...
/**
 * Получить информацию о группе (для админов)
 */
...
export async function getUserStats(): Promise<any> {
  const [totalUsers, activeTelegramUsers, totalGroups, totalSchedules] = await Promise.all([
    prisma.user.count(),
...
export async function getGigaChatToken(): Promise<string | null> {
  ...
  // Сначала пробуем получить ключ из настроек БД
...
/**
 * Обработать запрос пользователя через GigaChat
 */
