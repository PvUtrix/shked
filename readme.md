...
**ШКЕД** — это современная веб-платформа для управления расписанием занятий в университете с интегрированным Telegram ботом и LLM поддержкой. Разработана с использованием Next.js 14, TypeScript, PostgreSQL, Prisma ORM и GigaChat. Система предоставляет удобный интерфейс для администраторов и студентов, позволяя эффективно управлять учебным процессом, группами, подгруппами и расписанием занятий.
...
- 📖 **Управление предметами** — добавление дисциплин с указанием преподавателей
...
- 🔐 **Управление пользователями** — администрирование учетных записей студентов и преподавателей
...
- `role` — роль пользователя (`admin` или `student`)
- `groupId` — связь с группой
- Связи: `accounts`, `sessions`, `group`, `userGroups`
...
После запуска seed скрипта будут созданы тестовые пользователи:
